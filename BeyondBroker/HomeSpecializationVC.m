...
@interface HomeSpecializationVC ()

@end

@implementation HomeSpecializationVC
...
- (void)viewDidLoad {
    ...
    NSArray *arrSelectedHomeTypes =[[UserDefault valueForKey:@"homeTypesIds"] componentsSeparatedByString:@","];
    self.arrSelectedId =[[NSMutableArray alloc] initWithArray:arrSelectedHomeTypes];
    
    [self performSelector:@selector(getHomeDetailsFromServer) withObject:nil afterDelay:0.5];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.lblTitle.text=CHECK_NULL_STRING([dict valueForKey:@"propertytype"]);
    
    if ([self.arrSelectedId containsObject:[dict valueForKey:@"propertytypeid"]]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *tempId =[dict valueForKey:@"propertytypeid"];
...
-(IBAction)goBack:(id)sender{
    ...
    if (self.arrSelectedId.count<1) {
       ...
       // [[NSUserDefaults standardUserDefaults] setValue:commaSepIds forKey:@"homeTypesIds"];
       // [[NSUserDefaults standardUserDefaults] synchronize];
       // [[NSUserDefaults standardUserDefaults] setBool:true forKey:@"HomeSpecialization"];
        ...
        NSString *post = [NSString stringWithFormat: @"agentid=%@&token=%@&save_type=1&property_type=%@",CHECK_NULL_STRING([[NSUserDefaults standardUserDefaults] valueForKey:@"loginUserId"]),
                          STATIC_TOKEN,commaSepIds];
...
-(void)getHomeDetailsFromServer{
    ...
    [[WebServiceCall getInstance] sendRequestWithUrl:[MAIN_URL stringByAppendingPathComponent:@"Agent_Webservice/agent_propertytype"] withData:postData withMehtod:@"POST" withLoadingAlert:@"Loading.." withCompletionBlock:^(BOOL success, NSData *response, NSError *error) {
